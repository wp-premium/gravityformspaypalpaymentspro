...
class RGPayPalPaymentsProUpgrade{

    public static function set_version_info($version_info){
        if ( function_exists('set_site_transient') )
            set_site_transient("gforms_paypalpaymentspro_version", $version_info, 60*60*12);
        else
            set_transient("gforms_paypalpaymentspro_version", $version_info, 60*60*12);
    }

    public static function check_update($plugin_path, $plugin_slug, $plugin_url, $offering, $key, $version, $option){
        $version_info = function_exists('get_site_transient') ? get_site_transient("gforms_paypalpaymentspro_version") : get_transient("gforms_paypalpaymentspro_version");
    ...
    public static function display_changelog($offering, $key, $version){
        ...
        if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code']){
            $page_text = sprintf(__("Oops!! Something went wrong.%sPlease try again or %scontact us%s.", 'gravityformspaypalpaymentspro'), "<br/>", "<a href='http://www.gravityforms.com'>", "</a>");
        }
        else{
